...
class QuantizeParamsWithCalibrationTest(parameterized.TestCase):
  ...
  def test_delegates_to_quantize_fn(self):
    """Tests that quantize_fn is called with a properly prepared weight."""
    rules = [gptq.GptqRule(module_path='Dense_0', weight_qtype=jnp.int8)]
    model, ptq_model, x, variables, abs_variables = (
        self._setup_model_and_stats(rules)
    )

    prepared_weights = []

    def mock_quantize(prepared):
      prepared_weights.append(prepared)
    ...
    self.assertLen(prepared_weights, 1)
    pw = prepared_weights[0]
    self.assertEqual(pw.weight.ndim, 2)
    self.assertIn('hessian', pw.calibration_stats)
    self.assertEqual(pw.path[-1], 'kernel')
  ...
  def test_without_ptq_fallback_returns_partial_tree(self):
    ...
    self.assertIn('Dense_0', result)
    self.assertNotIn('Dense_1', result)
