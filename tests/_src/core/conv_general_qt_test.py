...
class ConvGeneralQtTest(parameterized.TestCase):
  ...
  def test_grad_against_fq(
    ...
    if (
      ...
      # (4th) diverges to ~1.29 on the XLA CPU backend (vs ~0.007 on TPU),
      # while every other component stays in range. A tolerance that large
      # would make the assertion meaningless, so skip only this specific
      # parameterization off TPU.
