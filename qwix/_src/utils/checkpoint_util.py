...
def _sharding_from_template_metadata(
  ...
  if not isinstance(boxed_template_value, nnx.Variable):
    return None
  # Reuse flax's own per-variable helper so the reconstructed PartitionSpec
  # (including how `None` entries and logical-axis rules are handled) matches
  # nnx.get_named_sharding exactly.
