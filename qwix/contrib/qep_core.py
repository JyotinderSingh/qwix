...
from qwix.contrib.gptq_core import cholesky_inverse
...
def weight_correct(
  ...
  The correction formula is:
    W_corrected = W + perccorr * (W @ H_delta @ H_inv)
  ...
  Args:
    ...
    H_delta: Cross-correlation matrix from compute_qep_stats,
      shape (columns, columns).
    perccorr: Weight correction factor. 0.0 = no correction, 1.0 = full
      correction. Default 0.5 per QEP paper recommendations.
    percdamp: Dampening factor for Hessian inversion as a fraction of the
      average diagonal. Default 1.0 (much higher than GPTQ's 0.01 because
      this inversion needs more regularization).
