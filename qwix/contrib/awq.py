...
def quantize_params(
  ...
  def _quantize(prepared: calibration.PreparedWeight) -> Any:
    activation_scale = prepared.calibration_stats['act_scale']
    assert activation_scale.shape[0] == prepared.weight.shape[1]

    # Quantize the weight with AWQ.
    w_q, scales = awq_core.quantize_weight(
        prepared.weight, activation_scale, prepared.how, n_grid=n_grid
    )

    # Restore original shape for QArray.
    w_q = prepared.restore_shape(w_q)
    ...
    return WithAwqScale(
        ...
        contracting_axis=prepared.contracting_axis,
        how=prepared.abs_w.how,
    )
